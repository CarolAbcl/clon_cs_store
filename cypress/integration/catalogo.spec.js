describe('Catalogo desktop', () => {
  ...
  it('deberia aumentar la cantidad de producto', () => {
    // button +
    cy.get('.ProductCard:first-child .containerInfoProduct div button:nth-of-type(2)').click({ force: true })

    // Input qty
    cy.get('.ProductCard:first-child .containerInfoProduct div #quantity').should('have.value', '1')
  })

  it('deberia disminuir la cantidad de producto', () => {
    // setea el input #quantity con 1
    cy.get('.ProductCard:nth-child(2) .containerInfoProduct div #quantity').type('1', { force: true })

    // button -
    cy.get('.ProductCard:nth-child(2) .containerInfoProduct > div:nth-child(2) button:nth-of-type(1)').click({
      force: true,
    })

    // Input qty
    cy.get('.ProductCard:nth-child(2) .containerInfoProduct div #quantity').should('have.value', '0')
  })

  it('deberia mostrar informacion de precios del producto', () => {
    // button i (info)
    cy.get('.ProductCard:nth-child(1) .containerInfoProduct .container .flex button').click({ force: true })
...
describe('Catalogo mobile', () => {
  ...
  it('deberia cerrar sidebar menu al presionar el background', () => {
    cy.get('.burger_container').click()
    cy.get('.content').should('be.visible')
    cy.get('.navbar > .background').click({ force: true })
    cy.get('.content').should('not.be.visible')
  })
  it('deberia cerrar sidebar al seleccionar una opcion del menu', () => {
    cy.get('.burger_container').click()
    cy.get('.content').should('be.visible')
